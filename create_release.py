...
def parse_cmake_file(cmake_file, current_dir):
   ...
   for line in cmake_file.split('\n'):
      if 'add_subdirectory' in line:
         ...
         final_cmake += parse_cmake_file(open_file(os.path.join(current_dir.peek(), 'CMakeLists.txt')), current_dir)
         current_dir.pop()
      elif 'include' in line:
         substr = re.findall(r'\((.*?)\)', line)
         new_cmake_file = open_file(os.path.join(current_dir.peek(), substr[0]))
